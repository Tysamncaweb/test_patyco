...
class PosConfig(models.Model):
    ...
    @api.depends('l10n_es_simplified_invoice_sequence_id.number_next_actual',
                 ...
                 'l10n_es_simplified_invoice_sequence_date_range.date_from',)
    def _compute_simplified_invoice_sequence(self):
        for pos in self:
            seq = pos.l10n_es_simplified_invoice_sequence_id
            seq1 = self.env['ir.sequence.date_range'].search([('sequence_id', '=', seq.id)])
            ...
            pos.l10n_es_simplified_invoice_dian_type = seq.dian_type
            pos.l10n_es_simplified_invoice_date_from = seq1.date_from
            pos.l10n_es_simplified_invoice_number_next_actual = seq1.number_next_actual
            pos.l10n_es_simplified_invoice_dian_range_hasta = seq1.dian_range_hasta

    iface_l10n_es_simplified_invoice = fields.Boolean(
        string='Use simplified invoices for this POS',
        default=True,
    )
    l10n_es_simplified_invoice_sequence_date_range = fields.Many2one(
    ...
    l10n_es_simplified_invoice_limit = fields.Float(
        ...
        help='Over this amount is not legally posible to create '
             'a simplified invoice',  # Spanish legal limit
    ...
    l10n_es_simplified_invoice_dian_type = fields.Integer(
        'Simplified Invoice dian type',
        readonly=True,
        compute='_compute_simplified_invoice_sequence',
    )
    l10n_es_simplified_invoice_date_from = fields.Date(
        'Simplified Invoice date from',
        readonly=True,
        compute='_compute_simplified_invoice_sequence',
    )

    l10n_es_simplified_invoice_number_next_actual = fields.Integer(
        'Simplified Invoice number next actual',
        readonly=True,
        compute='_compute_simplified_invoice_sequence',
    )
    l10n_es_simplified_invoice_dian_range_hasta = fields.Integer(
        'Simplified Invoice dian range hasta',
        readonly=True,
        compute='_compute_simplified_invoice_sequence',
    )
    ...
    def _get_default_padding(self):
        return self.env['ir.config_parameter'].get_param(
            'paty_pos.simplified_invoice_sequence.padding', 4)

    def _get_default_prefix(self):
        return self.env['ir.config_parameter'].get_param(
            'paty_pos.simplified_invoice_sequence.prefix', '')
