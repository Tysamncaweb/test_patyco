...
class GenerarEtiqueta(models.Model):
    _name = 'coordinadora.generate.label'
    _description = "Generar etiquetas Nacionales e Internacionales (Rotulos)"

    name = fields.Char()
    date = fields.Date('Fecha')
    state = fields.Selection([('draft', 'Borrador'), ('generate', 'Etiqueta Generada')], default='draft')
    ...
    def name_get(self):
        ...
        for number in self:
            if self.number_guia:
                res.append((number.id, 'Etiqueta de la Guía N° %s' % (number.number_guia)))
            else:
                res.append((number.id, 'Etiqueta de la Guía N° '))
        return res
    ...
    def unlink(self):
        for order in self:
            if order.state not in ('draft'):
                ...
                return super(GenerarEtiqueta, order).unlink()
        return
    ...
    def write(self, values):
        if values.get('number_guia'):
            ...
            self.validate_exitencia(values.get('number_guia'))
        #data_general(self,values)
        res = super(GenerarEtiqueta, self).write(values)
        return res
    ...
    def create(self, values):
        ...
        self.validate_exitencia(values.get('number_guia'))
        data_general(self,values)
        res = super(GenerarEtiqueta, self).create(values)
        return res


    def validate_exitencia(self,number_guia):
        valide = self.env['coordinadora.generate.label'].search([('number_guia','=',number_guia)])
        if valide:
            raise UserError(
                'Esta Etiqueta de la Guía %s, ya fue generado' % (number_guia))
        else:
            return True
